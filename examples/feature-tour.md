...
Open `tables/graph.csv` in the Files panel. The spreadsheet lets you
edit the CSV, **Insert table** adds its current contents as a Markdown table,
and the Mermaid action lets you select a CSV source for a live diagram. CSV cell references such as
...
The same Mermaid action is available in the document toolbar. It first asks
for the CSV source, then for the Mermaid template.

## CSV-backed plots

Open `tables/plot.csv` in the Files panel to try a linked Mermaid
...
The linked Mermaid expansion takes each line's name from the CSV column header
and adds it only to the first point.
...
* Export the styled document as self-contained HTML or through the print dialog
  as PDF. Both preserve the configured page size and the editor's page breaks.
